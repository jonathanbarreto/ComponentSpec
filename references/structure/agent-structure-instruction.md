...
### Consolidated Typography Table (required when ≥2 text elements)

When the component contains two or more distinct text elements (label + input value + hint text; title + subtext; primaryLabel + secondaryLabel; …), the per-element typography rows scattered across different dimensional sections do not give an engineer a single place to see the typographic system. Emit a consolidated summary in addition to the per-section rows.

One row per distinct text element, with the columns below:

```jsonc
{
  "element": "Label title",                  // engineer-readable name (prefer the engineering-API name, e.g. "label", "hintText", "placeholder"; fall back to the Figma layer name only when no API name applies)
  "family": "Uber Move",                     // font family
  "weight": "Medium",                        // weight name (Regular / Medium / Bold / …) or numeric weight when the style uses one
  "size": 14,                                // font size in design units (plain number, no units)
  "lineHeight": 18,                          // line height in design units, or "auto" when unset
  "letterSpacing": 0.38,                     // letter spacing in design units, or 0 when unset
  "styleId": "S:abc123…",                    // Figma text style id when bound, null when inline
  "styleName": "Body/S Medium",              // resolved text style name when styleId is set, null when inline
  "notes": "Truncates at 1 line; ellipsis." // optional; truncation / wrapping / language-specific behavior only
}
```

Rules:

- Include every text element that renders in the default variant of the default size. Include text elements that are only visible in non-default states (e.g., `hintText` visible only when `validationState !== 'none'`) as long as they carry a typographic style — add a short clarifier in `notes`.
- When two text elements share the exact same style (same family + weight + size + lineHeight + letterSpacing, and either the same `styleId` or both inline), still emit one row per element — the consolidated table is an element-level index, not a style-level one.
- Skip the artifact when the component has ≤1 distinct text element; the per-section typography rows are sufficient.

When running under the `extract-structure` skill, emit this as `_extractionArtifacts.typographyTable[]` on the cache JSON. When running as the standalone `create-structure` skill, render this as a separate typography panel beside the dimensional sections; no JSON file is written. The per-section typography rows continue to exist — this artifact complements them, it does not replace them.
...
### Visual-only axis
Same children, same dimensional properties. Only fills, effects, opacity, or colors change. **Skip the dimensional-table section — but still capture per-state deltas as an artifact.**

Even when an axis does not change dimensions, it often flips non-dimensional properties that an engineer must implement: `strokeWeight` appearing or disappearing, child visibility toggling (e.g., a cursor indicator only in `focused`), an iconGlyph swap (e.g., `circle_i` → `alert` when validation fails), or a `textContent` switch (e.g., placeholder → error message). Color alone cannot communicate these. Capture them in a structured delta artifact instead of paraphrasing them in prose.

For every visual-only axis, scan each element in the default variant's tree against every off-default value of the axis, and record any non-dimensional property that changes. Emit one artifact entry per axis:

```jsonc
{
  "axis": "state",
  "columns": ["rest", "active", "read-only", "success", "error", "disabled", "filled"],
  "rows": [
    { "element": "Input strokeWeight",  "values": [1, 1, 1, 1, 1, 0, 1],                                                            "notes": "Disabled removes the border entirely." },
    { "element": "Cursor indicator",    "values": ["hidden", "visible", "hidden", "hidden", "hidden", "hidden", "hidden"],         "notes": "Only visible when focused." },
    { "element": "Hint icon glyph",     "values": ["circle_i", "circle_i", "circle_i", "circle_i", "alert", "circle_i", "circle_i"], "notes": "Alert glyph replaces info glyph on validation error." }
  ]
}
```

Columns come from the axis's Figma option list (or from `stateAxisMapping[].runtimeCondition` when the axis was decomposed by the API layer — the renderer handles the relabeling). Rows are one non-dimensional property per affected element; `values[]` is the per-column measurement for that property. Skip rows where every value is identical — there is no delta to record. Dimensional properties are out of scope here; they belong in the regular dimensional sections.

When running under the `extract-structure` skill, emit this as `_extractionArtifacts.visualOnlyAxisDeltas[]` on the cache JSON. When running as the standalone `create-structure` skill, render this as an additional annotation panel beside the dimensional sections; no JSON file is written. The rule is additive — it does not change where dimensional tables live.

### AI reasoning for edge cases
- A 0.5px rounding difference is noise — skip it.
- `strokeWeight` going from 0 to 1 is meaningful — document it as both a dimensional row (when present in a dimensional section) and a `visualOnlyAxisDeltas[]` row (when it flips across a visual-only axis).
...
### Vocabulary: describing `SPACE_BETWEEN` / distributed layouts

When Figma reports `primaryAxisAlignItems === "SPACE_BETWEEN"` on an auto-layout container, the rendered gap between children is computed from the container's available width minus the sum of child widths — it is never a fixed value. **Do not** describe this with vague jargon like "layout-distributed", "evenly distributed", or "auto-spaced"; those phrases lead engineers to guess a pixel value that won't survive a container resize. Use the required note template on the `itemSpacing` (or `contentSpacing`) row:

> `primaryAxisAlignItems=SPACE_BETWEEN; the rendered value is width-dependent and must not be hard-coded.`

In the dimensional column, write `"—"` (with `provenance: "measured"`) instead of the Figma-reported pixel value, because that value is an artifact of the default-variant width and is not a design constraint. The same rule applies to `counterAxisAlignItems: "SPACE_BETWEEN"` (vertical distribution).
...
### Sub-component variant walks (`subComponentVariantWalks`)

When a constitutive sub-component carries its own variant axes (e.g., `Input` has `size: large | medium | small`), the parent's `_base.json.variants[*]` only captures the child at whatever variant the designer placed in the parent — usually one. Plugin Phase I closes that gap by walking the child's OWN component set across the cross-product of its axes and emitting the results under `_base.json.subComponentVariantWalks[subCompSetId]`. Shape:

```jsonc
"subComponentVariantWalks": {
  "<subCompSetId>": {
    "name": "<child instance name>",
    "subCompSetName": "<parent COMPONENT_SET name or null>",
    "axes": { "<axisName>": ["<option>", "..."] },
    "variants": [
      {
        "variantKey": "<axis=value|axis=value>",
        "variantProperties": { "<axisName>": "<value>" },
        "dimensions": { /* collapsed {value, token, display} */ },
        "treeHierarchical": { /* dims-only walk of the variant's body; stops at nested INSTANCE boundaries */ }
      }
    ],
    "skipped": true,            // only when cross-product exceeded Phase I's cap
    "skippedReason": "<string>" // parallel to skipped
  }
}
```

**Read-path rule.** When emitting a spec section for a constitutive sub-component whose columns correspond to its OWN variant axis (e.g., an `Input` section with `Large / Medium / Small` columns), every column MUST be populated from `subComponentVariantWalks[subCompSetId].variants[*]`:

1. Look up the block for this sub-component by its `subCompSetId`.
2. For each column, find the `variants[*]` entry whose `variantProperties` match the column header (the column axis value — case-insensitive, whitespace-normalised).
3. Fill cell values from that entry's `dimensions` (for the root row) or from its `treeHierarchical` descendants (for nested-frame rows, looked up by matching `name` along the path).
4. Cells filled this way carry `provenance: "measured"` — the walk is a first-class Figma measurement, not a delta-script fallback.

**When to emit `"—"` with `provenance: "not-measured"`.** Only in two cases:
- The block is present but `skipped: true` — the cross-product exceeded Phase I's cap. Surface the `skippedReason` verbatim in the row's `notes`.
- The block is missing a specific `variants[*]` entry (rare; indicates the COMPONENT_SET had no matching variant). Cite the missing `variantKey` in the row's `notes`.

**`_deltaExtractions` policy.** A `_deltaExtractions[*]` gap entry is only emitted when the above Phase I data is absent for an axis the section displays — i.e., legacy `_base.json` files produced before Phase I shipped, or entries that were skipped. Do NOT emit a delta gap when Phase I walked the variants successfully: the data is already in hand.

**Legacy fallback.** When `subComponentVariantWalks` is absent from `_base.json` entirely (legacy fixture), continue the previous behavior: fill the default-variant column from the embedded instance's `dimensions`, emit `"—"` with `provenance: "not-measured"` in the other columns, and log a single `_deltaExtractions` entry with `unavailable: "no-figma-link"` (or "legacy _base.json — rerun plugin" when figmaLink is available).
...
- **Symmetric padding emits both axes.** A captured `padding: { vertical: <v>, horizontal: <h> }` emits BOTH a `verticalPadding` row and a `horizontalPadding` row even when one side is `0`. The zero row is the witness that the asymmetry is intentional.
- **"Absent" ≠ "zero".** A missing row means the extractor didn't look; a row with value `0` means the extractor looked and found zero. Never let the first masquerade as the second.
...
| `_layerName` / `_layerId` | **Required** — the literal Figma layer name + node id this group corresponds to. See **Stamping layer identity on group-header rows** below. |
...
- `Container` — values: – | – | – — notes: "Tap target" — `_layerName: "Container"`, `_layerId: "10225:15388"` (group header, no `isSubProperty`)
...
**When `_layerName` cannot be resolved.** When the group describes a logical zone with no corresponding single Figma layer (rare; usually a sign the group should be reorganised), emit `_layerName: "<original spec>"` and `_layerId: null`, and add a one-line note in the row's `notes` explaining the missing pinning. Both fields remain present; downstream tooling reads `_layerId === null` as a render-meta gap and surfaces it.
...
| ☐ **Sub-component variant walks consumed** | For every section whose columns correspond to a constitutive sub-component's OWN variant axis (e.g., `Input` displayed at `Large / Medium / Small`), every column is populated from `_base.json.subComponentVariantWalks[subCompSetId].variants[*]` with `provenance: "measured"`. `"—"` cells are permitted ONLY when the entry is `skipped: true` (cite `skippedReason` in notes) or when a specific `variants[*]` combo is genuinely missing (cite the missing `variantKey` in notes). A `_deltaExtractions` gap entry is only emitted when `subComponentVariantWalks` is missing or skipped — never when Phase I walked the axis successfully. |
...
| ☐ **Visual-only axis deltas captured** | For every axis classified as visual-only, a `visualOnlyAxisDeltas[]` entry has been emitted (extract path) or an annotation panel has been rendered (create path), listing every element whose `strokeWeight`, visibility, `iconGlyph`, or `textContent` differs across the axis. Rows with identical values across every column are omitted. |
| ☐ **Consolidated typography table emitted** | When the component has ≥2 distinct text elements, `typographyTable[]` has one row per element with `element`, `family`, `weight`, `size`, `lineHeight`, `letterSpacing`, `styleId`, and `styleName` populated. Skipped only when the component has ≤1 text element. |
| ☐ **SPACE_BETWEEN described correctly** | No row notes or `generalNotes` use the phrase "layout-distributed", "evenly distributed", or "auto-spaced". Every `SPACE_BETWEEN` container uses the required note template `primaryAxisAlignItems=SPACE_BETWEEN; the rendered value is width-dependent and must not be hard-coded.` and a `"—"` dimensional value. |
